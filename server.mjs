...
// The distributed build uses PKCE and deliberately does not bundle a client
// secret. A local OAuth JSON file is supported only for developer compatibility
// and is never written to source, UI, logs, or packaged resources.
const bundledGoogleClientId = '658472864580-mp1kghp3n1i2p9mtojocmaf36v5q9uus.apps.googleusercontent.com';
let bundledGoogleClientSecret = '';
...
async function loadBundledOAuthClient(){
  const candidates=[
    process.resourcesPath?join(process.resourcesPath,'google-oauth-client.json'):'',
    join(root,'google-oauth-client.json')
  ].filter(Boolean);
  for(const filePath of candidates){
    try{
      const parsed=JSON.parse(await readFile(filePath,'utf8'));
      const client=parsed.installed||parsed.web||parsed;
      if(client?.client_id===bundledGoogleClientId&&client?.client_secret){
        bundledGoogleClientSecret=String(client.client_secret);
        return true;
      }
    }catch{}
  }
...
// This is a desktop OAuth client. Keep the client identity inside the app and
// deliberately ignore legacy env values so an old .env.local cannot override
// the client or redirect URI on a tester's machine.
// Google desktop OAuth uses a loopback redirect and PKCE. A client secret is
// included only when a developer has a local OAuth JSON file; the distributed
// installer deliberately runs without one.
function oauthConfig(){return{clientId:bundledGoogleClientId,clientSecret:bundledGoogleClientSecret,redirectUri:`http://localhost:${Number(process.env.PORT||port)}/oauth/callback`}}
function googleScopes(){return['openid','email','profile','https://www.googleapis.com/auth/classroom.courses.readonly','https://www.googleapis.com/auth/classroom.coursework.me','https://www.googleapis.com/auth/classroom.courseworkmaterials.readonly','https://www.googleapis.com/auth/classroom.announcements.readonly','https://www.googleapis.com/auth/classroom.topics.readonly','https://www.googleapis.com/auth/drive.file','https://www.googleapis.com/auth/drive.readonly','https://www.googleapis.com/auth/calendar.events.readonly'].join(' ')}
async function refreshToken(token){if(!token?.refresh_token||!token.expires_at||Date.now()<token.expires_at-60000)return token;const cfg=oauthConfig();const params={client_id:cfg.clientId,grant_type:'refresh_token',refresh_token:token.refresh_token};if(cfg.clientSecret)params.client_secret=cfg.clientSecret;const r=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(params),signal:AbortSignal.timeout(15000)});if(!r.ok)throw new Error(`Google token refresh ${r.status}`);const next=await r.json();const updated={...token,...next,expires_at:Date.now()+Number(next.expires_in||3600)*1000};await saveToken(updated);return updated}
const classroomContentCache=new Map();
let classroomTasksInFlight=null;
let classroomTasksCache=null;
let classroomTasksCacheAt=0;
const waitMs=ms=>new Promise(resolve=>setTimeout(resolve,ms));
async function googleFetch(url,token){const fresh=await refreshToken(token);for(let attempt=0;attempt<3;attempt++){const r=await fetch(url,{headers:{Authorization:`Bearer ${fresh.access_token}`},signal:AbortSignal.timeout(15000)});if(r.ok)return r.json();const detail=await r.text();const parsed=(()=>{try{return JSON.parse(detail)}catch{return{}}})();const message=parsed.error?.message||parsed.error?.status||detail.slice(0,180);if((r.status===429||r.status>=500)&&attempt<2){const retryAfter=Number(r.headers.get('retry-after')||0);await waitMs(Math.min(3000,retryAfter>0?retryAfter*1000:500*(attempt+1)));continue}throw new Error(`Google API ${r.status}: ${message}`)}}
...
function plannerPrompt(input){
  const source=truncateStudyText(input.classroomContext||'',26000);
  return `คุณเป็นผู้ช่วยวางแผนงานนักเรียนที่ต้องยึดหลักฐานเท่านั้น วิเคราะห์งานจาก Google Classroom ที่แนบด้านล่างอย่างละเอียด โดยห้ามเดาคำสั่ง งานย่อย จำนวนชิ้นงาน หรือกำหนดเวลาที่ไม่มีในข้อมูล
ตอบเป็น JSON เท่านั้น ห้ามมี Markdown หรือข้อความนอก JSON ตามรูปแบบนี้:
{"tasks":[{"id":"รหัสงาน","estimatedMinutes":45,"difficulty":"easy|medium|hard","subtasks":["งานย่อยที่ระบุได้จากข้อมูลเท่านั้น"],"rationale":"เหตุผลที่ตรวจสอบได้","evidence":["ข้อความจริงจากรายละเอียดงานหรือไฟล์แนบ"],"confidence":"high|medium|low","unreadableSources":["ชื่อไฟล์ที่อ่านไม่ได้"]}],"globalNotes":["ข้อจำกัดที่พบ"]}
กติกา: estimatedMinutes ต้องเป็นจำนวนเต็ม 20–360 และอิงจากจำนวนขั้นตอน/ชิ้นงาน/ความซับซ้อนที่เห็นจริง ถ้ารายละเอียดไม่พอให้ใช้ confidence=low ใช้ค่าประมาณแบบอนุรักษ์นิยม และเขียนเหตุผลว่าข้อมูลไม่ครบ ห้ามสร้างหลักฐานหรือเลขหน้า หากไฟล์แนบอ่านไม่ได้ให้ใส่ชื่อไฟล์ใน unreadableSources และอย่าใช้เนื้อหาจากงานอื่นแทน
กิจวัตรและ Preference ของผู้ใช้สำหรับใช้ประกอบเหตุผล:
${String(input.message||'ไม่ได้ระบุ')}
ข้อมูลจาก Classroom และไฟล์แนบที่อ่านได้:
---
${source||'ไม่มีข้อมูลที่อ่านได้'}
---`;
}
...
function qwenOutputLimit(input){const count=qwenRequestedCount(input);if(input.action==='planner-analysis')return 5200;if(input.action==='summary')return 5000;if(input.action==='chat')return 1800;if(input.action==='quiz')return Math.min(6500,Math.max(3600,count*700));return Math.min(6000,Math.max(3000,count*520))}
...
async function generateWithAI(input){
 ...
 async function getClassroomTaskDetail(courseId,courseWorkId,token){const item=await googleApi(`courses/${courseId}/courseWork/${courseWorkId}`,token);let submission=null;try{const data=await googleApi(`courses/${courseId}/courseWork/${courseWorkId}/studentSubmissions?userId=me&pageSize=10`,token);submission=data.studentSubmissions?.[0]||null}catch{}return{courseId,courseWorkId,topicId:item.topicId?String(item.topicId):'',title:item.title||'งานจาก Google Classroom',description:item.description||'',dueDate:item.dueDate||null,dueTime:item.dueTime||null,maxPoints:item.maxPoints??null,workType:item.workType||'',alternateLink:item.alternateLink||null,materials:item.materials||[],submission}}
function plannerTaskId(task){return String(task?.courseWorkId||task?.id||task?.title||'งานไม่ทราบชื่อ')}
function plannerDueAt(task){const date=task?.dueDateRaw||task?.dueDate,time=task?.dueTimeRaw||task?.dueTime;if(date&&date.year&&date.month&&date.day)return new Date(Date.UTC(Number(date.year),Number(date.month)-1,Number(date.day),Number(time?.hours||0),Number(time?.minutes||0))).toISOString();const parsed=Date.parse(String(task?.deadline||''));return Number.isNaN(parsed)?null:new Date(parsed).toISOString()}
function plannerLocalAnalysis(task,detail,readableSources=[]){
  const source=detail||task||{},description=String(source.description||'').trim(),title=String(source.title||task?.title||'งานจาก Google Classroom'),combined=`${title}\n${description}`.trim();
  const evidence=[];if(description)evidence.push(description.replace(/\s+/g,' ').slice(0,420));
  const readableText=readableSources.filter(item=>item.status==='readable'&&item.textSnippet).map(item=>`${item.title}: ${item.textSnippet}`).join(' ').trim();
  if(readableText)evidence.push(readableText.slice(0,420));
  const unreadableSources=readableSources.filter(item=>item.status==='unavailable').map(item=>item.title).filter(Boolean);
  const materials=Array.isArray(source.materials)?source.materials:[];if(materials.length)evidence.push(`มีสื่อหรือไฟล์แนบ ${materials.length} รายการ`);
  const explicit=combined.match(/(\d{1,3})\s*นาที/i);let minutes=explicit?Number(explicit[1]):45;
  if(/รายงาน|โครงงาน|ชิ้นงาน|มายแมพ|โปสเตอร์|essay|project/i.test(combined))minutes+=45;
  if(/วิเคราะห์|เปรียบเทียบ|อธิบายเหตุผล|หลายขั้นตอน|อย่างละเอียด|อ้างอิง/i.test(combined))minutes+=30;
  if(materials.length>=2)minutes+=20;
  if(readableText.length>900)minutes+=20;
  if(readableText.length>3000)minutes+=30;
  if(/แบบฝึกหัด|worksheet|ใบงาน|โจทย์/i.test(combined))minutes+=15;
  minutes=Math.max(20,Math.min(240,minutes));
  const difficulty=minutes>=120||/วิเคราะห์|โครงงาน|รายงาน|หลายขั้นตอน|อย่างละเอียด/i.test(combined)?'hard':minutes>=70||materials.length>0?'medium':'easy';
  const subtasks=description.split(/\r?\n|[•●▪]/).map(x=>x.replace(/^\s*[-*]\s*/,'').trim()).filter(x=>x.length>=8).slice(0,6);
  if(!subtasks.length){if(/รายงาน|โครงงาน|ชิ้นงาน|essay|project/i.test(combined))subtasks.push('อ่านคำสั่งและกำหนดขอบเขตงาน');if(materials.length)subtasks.push('อ่านสื่อหรือไฟล์แนบที่ระบบเข้าถึงได้');subtasks.push('ลงมือทำและตรวจความครบถ้วนก่อนส่ง')}
  const confidence=description&&(!unreadableSources.length||evidence.length>1)?'medium':'low';
  const rationale=description||readableText?`ประเมินประมาณ ${minutes} นาทีจาก${description?'คำสั่งงาน':''}${description&&readableText?' และ ':''}${readableText?'ข้อความที่อ่านได้จากไฟล์แนบ':''}${materials.length?` รวมสื่อแนบ ${materials.length} รายการ`:''} โดยกันเวลาอ่านคำสั่ง/ตรวจงานก่อนส่ง`:'ยังมีรายละเอียดงานไม่พอ จึงใช้เวลาประมาณแบบอนุรักษ์นิยมและไม่เดาข้อกำหนดเพิ่มเติม';
  return{id:plannerTaskId(task),courseId:String(source.courseId||task?.courseId||''),courseWorkId:String(source.courseWorkId||task?.courseWorkId||''),title,estimatedMinutes:minutes,difficulty,subtasks,rationale,evidence:evidence.slice(0,4),confidence,unreadableSources,dueAt:plannerDueAt(source)||plannerDueAt(task),readableSources};
}
async function readTaskMaterialsForPlanner(detail,token){const sources=[];for(const material of detail.materials||[]){const drive=material.driveFile?.driveFile||material.driveFile,linkedId=material.link?.url?extractGoogleDriveId(material.link.url):'',fileId=drive?.id||linkedId,label=drive?.title||material.link?.title||material.youtubeVideo?.title||'ไฟล์แนบจาก Classroom';if(!fileId){sources.push({title:label,status:'unavailable',reason:'ลิงก์หรือวิดีโอยังไม่ได้ถอดเนื้อหา'});continue}try{const extracted=await getDriveMaterial(fileId,token);const readable=!extracted.unavailable; sources.push({title:extracted.title||label,status:readable?'readable':'unavailable',chars:extracted.text?.length||0,textSnippet:readable?String(extracted.text||'').replace(/\s+/g,' ').slice(0,1800):'',reason:extracted.unavailable||''})}catch(error){sources.push({title:label,status:'unavailable',reason:String(error?.message||'อ่านไฟล์ไม่ได้')})}}return sources}
async function analyzePlannerTasks(input,token){const submitted=Array.isArray(input.tasks)?input.tasks.slice(0,24):[];const refs=submitted.filter(task=>task.courseId&&task.courseWorkId);const results=[];for(let index=0;index<submitted.length;index++){const task=submitted[index];let detail=task,sources=[];if(token?.access_token&&task.courseId&&task.courseWorkId){try{detail=await getClassroomTaskDetail(task.courseId,task.courseWorkId,token);detail={...task,...detail};sources=await readTaskMaterialsForPlanner(detail,token)}catch(error){sources=[{title:'รายละเอียดจาก Google Classroom',status:'unavailable',reason:String(error?.message||'อ่านรายละเอียดไม่ได้')}]} }results.push(plannerLocalAnalysis(task,detail,sources))}return{source:'local-planner',aiUsed:false,tasks:results,globalNotes:['วิเคราะห์จากรายละเอียดและไฟล์ที่ระบบอ่านได้โดยตรง','ยังไม่ได้ส่งข้อมูลการเรียนไปยัง AI ภายนอก จึงไม่สร้างข้อมูลที่ไม่มีหลักฐาน','หากไฟล์แนบอ่านไม่ได้ ระบบจะไม่นำเนื้อหาจากไฟล์นั้นมาเดาแทน'],routine:String(input.routine||''),preference:String(input.preference||''),updatedAt:Date.now()}}
...
async function route(req,res){const url=new URL(req.url,`http://${req.headers.host}`);
  ...
  if(req.method==='GET'&&url.pathname==='/api/classroom/tasks'){const token=await readToken();if(token?.access_token){try{return json(res,200,{tasks:await getClassroomTasksWithRefsCached(token),source:'google-classroom'})}catch(e){return json(res,502,{tasks:[],source:'google-classroom',error:e.message})}}return json(res,200,{tasks:[],source:'demo'})}
  ...
  if(req.method==='GET'&&url.pathname==='/oauth/callback'){const state=url.searchParams.get('state'),code=url.searchParams.get('code'),request=oauthRequests.get(state);oauthRequests.delete(state);if(!request||!code){oauthLastError={stage:'callback',reason:'missing_code_or_expired_state'};return redirect(res,'/?google=error&reason=missing_code')}const cfg=oauthConfig();const tokenParams={client_id:cfg.clientId,code,code_verifier:request.verifier,grant_type:'authorization_code',redirect_uri:cfg.redirectUri};if(cfg.clientSecret)tokenParams.client_secret=cfg.clientSecret;const tokenResponse=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(tokenParams)});if(!tokenResponse.ok){const errorText=await tokenResponse.text();const errorJson=(()=>{try{return JSON.parse(errorText)}catch{return{}}})();oauthLastError={stage:'token_exchange',status:tokenResponse.status,reason:errorJson.error||'unknown',description:errorJson.error_description||''};console.error('Google OAuth token exchange failed',tokenResponse.status,errorText);return redirect(res,`/?google=error&reason=token_${tokenResponse.status}`)}const token=await tokenResponse.json();token.expires_at=Date.now()+Number(token.expires_in||3600)*1000;await saveToken(token);oauthLastError=null;return redirect(res,'/?google=connected')}
  if(req.method==='POST'&&url.pathname==='/api/auth/logout'){try{await writeFile(tokenPath,'{}','utf8')}catch{}classroomTasksCache=null;classroomTasksCacheAt=0;return json(res,200,{connected:false})}
  ...
  if(req.method==='POST'&&url.pathname==='/api/planner/analyze'){try{const input=await body(req),token=await readToken();return json(res,200,await analyzePlannerTasks(input,token))}catch(e){return json(res,502,{error:'planner_analysis_failed',message:String(e?.message||'วิเคราะห์งานไม่สำเร็จ')})}}
...
export function startServer(bindPort=port){return new Promise((resolve,reject)=>{httpServer.once('error',reject);httpServer.listen(bindPort,'127.0.0.1',()=>{const address=httpServer.address();console.log(`StudyFlow server listening on http://127.0.0.1:${address.port}`);resolve(httpServer)})})}
...
async function getClassroomTasksWithRefsCached(token){const now=Date.now(),tokenKey=String(token?.refresh_token||token?.access_token||'');if(classroomTasksCache&&classroomTasksCache.tokenKey===tokenKey&&now-classroomTasksCacheAt<20000)return classroomTasksCache.tasks;if(classroomTasksInFlight)return classroomTasksInFlight;classroomTasksInFlight=getClassroomTasksWithRefs(token).then(tasks=>{classroomTasksCache={tokenKey,tasks};classroomTasksCacheAt=Date.now();return tasks}).finally(()=>{classroomTasksInFlight=null});return classroomTasksInFlight}
